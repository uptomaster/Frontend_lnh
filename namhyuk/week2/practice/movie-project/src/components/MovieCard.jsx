...
const MovieCard = ({ movie, show, onClick }) => {
  ...
  return (
    ...
    <div
      className=" group relative rounded-lg overflow-hidden shadow-lg transition-transform duration-300 hover:z-10 bg-gray-100 text-white aspect-[2/3] cursor-pointer"
    ...
    >
      ...
      <img
        src={
          data.image?.medium || // TVMaze API 형식
          data.poster_path || // TMDB API 형식 (보통 /abc.jpg 형태)
          data.poster || // 커스텀 데이터
      ...
      <div className="absolute inset-0 bg-black/80 p-5 flex flex-col justify-end opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300">
        ...
        <p className="text-sm text-gray-400">
          📅 {data.premiered || data.releaseDate}
        </p>

        {/* 기존에 있던 출연진 정보 유지 */}
        {data.cast && (
          <p className="text-xs text-indigo-400 font-semibold truncate mt-1">
            출연: {data.cast}
          </p>
        )}
      </div>
